...
public class ProjectionNodeEval1 {
	ProjectionNode pn;
	int flag = 0;
	...
	public ProjectionNodeEval1(ProjectionNode pNode) {
		this.pn = pNode;
		int columnSize = pn.getColumns().size();
	}

	public List<Datum[]> projectionNode(ProjectionNode pn) throws CastError {
		if (pn.getChild().type.equals(PlanNode.Type.NULLSOURCE)) {
			...
			if (pn.getChild().type.equals(PlanNode.Type.JOIN)) {
				...
				ScanNodeEval1 scanNodeEval = new ScanNodeEval1();
				resultList = scanNodeEval.scanNodeFunction(scanNode);
				...
				SelectNodeEval1 selectNodeEval = new SelectNodeEval1(selectionNode);
				resultList = selectNodeEval.selectionNode(selectionNode);
				String columnName = pn.getColumns().toString();
				return_list = projectFn(columnName);
				return return_list;
			}
